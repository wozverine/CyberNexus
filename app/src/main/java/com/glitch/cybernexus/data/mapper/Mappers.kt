...
fun Product.mapToProductUI(favorites: List<Int>) = ProductUI(
    id = id ?: 1,
    title = title.orEmpty(),
    price = price ?: 0.0,
...
fun List<Product>.mapProductToProductUI(favorites: List<Int>) = map {
    ProductUI(
        id = it.id ?: 1,
...
fun List<ProductEntity>.mapProductEntityToProductUI() = map {
    ProductUI(
        id = it.productId ?: 1,
        title = it.title.orEmpty(),
        price = it.price ?: 0.0,
